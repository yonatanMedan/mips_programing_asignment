...
#print the counting results in a table
printTable:
	jal printTableHead #print table head
	jal print_instraction_count_rows  #print instraction count rows
...
count_registers_in_I_type_command:
	...
	#body
	move $s0,$a0 #store $a0 for later use
	jal count_rs #preform count on rs
	move $a0,$s0
	jal count_rt  #preform count on rt
	#end
	lw $s0,4($sp) #load $s0
	lw $ra,0($sp) #load returm addres
	addi $sp,$sp,8 #free stack space
...
print_instraction_count_rows:
	...
	#body
	#print R-type row
	la $a0,R_type_str #load row string
	jal printStr #print row string
	lw $a0,R_type #load counter
	jal printInt #print counter
	jal printNewLine #print new line
	#print lw row (same as above but with lw row)
	la $a0,lw_str
	jal printStr
	lw $a0,l_w
	jal printInt
	jal printNewLine
	#print sw row (same as above but with sw row)
	la $a0,sw_str
	jal printStr
	lw $a0,s_w
	jal printInt
	jal printNewLine
	#print b_eq row (same as above but with beq row)
	la $a0,beq_str
	jal printStr
	lw $a0,b_eq
	jal printInt
